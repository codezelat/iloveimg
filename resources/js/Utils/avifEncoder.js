    ...
    try {
        ...
        try {
            ...
            if (imageSource instanceof File || imageSource instanceof Blob) {
                ...
                fileName = imageSource.name || 'image';
            ...
            const avifBlob = new Blob([encodedImage.binary], { type: 'image/avif' });
...
export async function decodeAvif(arrayBuffer) {
    ...
    if (!isSupported) {
        throw new Error('Your browser does not support AVIF decoding. Please use Chrome 85+, Firefox 93+, or Safari 16+.');
    }
 ...
 */
export async function convertAvifToFormat(avifSource, targetFormat = 'jpeg', quality = 0.92) {
    const isSupported = await isAvifSupported();
    
    if (!isSupported) {
        throw new Error('Your browser does not support AVIF. Please use Chrome 85+, Firefox 93+, or Safari 16+ to convert AVIF files.');
    }
 ...
 */
export async function convertAvifFileToFormat(file, targetFormat = 'jpeg', quality = 0.92) {
...
export function getQualityPresets() {
    return {
        small: { quality: 30, speed: 6, description: 'Small file size, some quality loss' },
        balanced: { quality: 50, speed: 4, description: 'Balanced quality and size (recommended)' },
        high: { quality: 70, speed: 3, description: 'High quality' },
        lossless: { quality: 95, speed: 2, description: 'Near lossless quality' },
    };
}
